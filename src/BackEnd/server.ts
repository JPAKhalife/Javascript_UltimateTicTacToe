...
const host = process.env.REDIS_HOST || "localhost"; // The name of the database
const port = parseInt(process.env.REDIS_PORT || "6379"); // The port of the database

const ratelimiter = rateLimit({
  windowMs: parseInt(process.env.WINDOW_MS || "900000"), // Use the WINDOW_MS environment variable or default to 15 minutes
  max: parseInt(process.env.MAX || "100"), // limit each IP to 100 requests per windowMs
});
