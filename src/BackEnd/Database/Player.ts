...
export class Player extends RedisObject<PlayerData> {
     ...
     */
    static async create(username: string, lobbyID?: string): Promise<Player | null> {
        ...
        const player = new Player(playerID, playerData, DatabaseManager.getInstance().getRegularClient());
    ...
    static async getById(playerID: string): Promise<Player | null> {
        try {
            const player = new Player(playerID, { playerID, username: "", lobbyID: "" }, DatabaseManager.getInstance().getRegularClient());
            await player.load();
            return player;
        } catch (error) {
            return null;
