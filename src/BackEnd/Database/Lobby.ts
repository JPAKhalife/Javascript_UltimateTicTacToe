...
export class Lobby extends RedisObject<LobbyData> {
     ...
     */
    static async create(lobbyData: CreateLobbyData, creatorId: string): Promise<Lobby> {
        ...
        const lobby = new Lobby(lobbyId, data, DatabaseManager.getInstance().getRegularClient());
    ...
    async addPlayer(playerId: string): Promise<void> {
        ...
        if (this.get("playersJoined") >= this.get("playerNum") && !this.get("allowSpectators")) {
        ...
        await this.withTransaction(async multi => {
            ...
            const newState = newPlayersJoined >= this.get("playerNum") ? "ready" : "waiting";
