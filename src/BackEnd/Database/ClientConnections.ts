...
export function newConnection(ws: any, connectionID: string) {
  ...
  if (storedWs === ws) {
    ...
    console.error(
      `[Connections] Failed to store WebSocket object in activeWebsockets map`,
    );
  }
...
export function playerRegistered(connectionID: string, playerID: string) {
  ...
  if (activeWebsockets.has(connectionID)) {
    ...
    console.warn(
      `[Connections] Connection ID ${connectionID} NOT found in activeWebsockets map`,
    );
  }
 ...
 */
async function setExpiryOnConnectionSessions(
  connectionID: string,
): Promise<void> {
  let redisClient = DatabaseManager.getInstance().getRegularClient();
...
export function disconnect(key: string): void {
  ...
  const connectionID = key.startsWith(prefix)
  ...
  if (ws) {
    try {
      ...
      console.error(
        `Error closing WebSocket for connection ${connectionID}:`,
        error,
      );
    }
  } else {
...
export function getWebsocketObject(connectionID: string) {
  ...
  if (ws) {
    ...
    console.warn(
      `[Connections] No WebSocket object found for connection ID: ${connectionID}`,
    );
    return null;
  }
}
