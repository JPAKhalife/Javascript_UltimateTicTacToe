...
//This is the player authenticated request that contains information all player requests should have
export const PlayerRequest = BaseRequest.extend({
    playerID: z.string().uuid({ version: "v4" }) //The player ID for identification. UUIDv4.
});
export type PlayerRequest = z.infer<typeof PlayerRequest>; 

//This request is used when the player wants to search for a lobby
export const LobbySearchRequest = PlayerRequest.extend({
    ...
    maxResults: z.number().int().lte(VALIDATION.MAX_RESULTS).gte(0).optional(),
...
export const LobbyCreateRequest = PlayerRequest.extend({
