...
export default class TutorialScreen implements Menu {
  ...
  constructor(sketch: p5) {
    ...
    // Store just the image references
    this.tutorialImages = [tictacboard, tictacboard_two, tictacboard_three];

    // Store text content as arrays of strings with positions as percentages of canvas size
    this.paragraphTexts = [
      [
        [
          "The Bigtictactoe board consists of one large tictactoe grid.",
          0.5,
          0.1,
        ],
        [
          "Each slot in the grid contains one smaller tictactoe grid.",
          0.5,
          0.2,
        ],
        [
          "The goal of the game is to get three points in a row on the large board.",
          0.5,
          0.7,
        ],
      ...
      [
        [
          "To start, player one can choose anyone of the small grids to play in.",
          0.5,
          0.1,
        ],
        ["They are then able to mark anywhere in that small grid.", 0.5, 0.2],
        [
          "The next player will then be sent to the corresponding area on the large grid.",
          0.5,
          0.7,
        ],
        ["Press space to continue.", 0.5, 0.8],
      ...
      [
        [
          "The player can then choose any grid to play in if the grid they are sent to is taken.",
          0.5,
          0.1,
        ],
        [
          "When a small grid is won, it becomes unable to be played in.",
          0.5,
          0.2,
        ],
  ...
  public draw(): void {
    ...
    this.sketch.image(
      this.tutorialImages[currentScreenIndex],
      imgX,
      imgY,
      imgSize,
      imgSize,
    ...
    if (
      this.keylistener.listen() === KEY_EVENTS.SELECT &&
      !this.transitioning
    ) {
      this.startTransition();
    }
