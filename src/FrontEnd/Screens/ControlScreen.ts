...
export default class ControlScreen implements Menu {
    ...
    if (this.transitioning) {
      if (this.transitioningIn) {
        ...
        if (this.screenState[1] > 0) {
          ...
          if (this.screenState[0] >= 1) {
            // We have 2 screens (0 and 1)
  ...
  public draw(): void {
    ...
    if (this.screenState[0] === 0) {
      ...
      this.sketch.text(
        "Use the WASD and/or Arrow keys to navigate through menus and the Ultimate Tictactoe grid.",
        getCanvasSize() / 2,
        (getCanvasSize() / 5) * 2,
        (getCanvasSize() / 4) * 3,
        (getCanvasSize() / 4) * 1,
      ...
      this.sketch.fill(255, this.screenState[1]);
      this.sketch.text(
        "Press space to select in the menus or play a spot on the Ultimate Tictactoe Grid.",
        getCanvasSize() / 2,
        (getCanvasSize() / 5) * 2 - 10,
        (getCanvasSize() / 4) * 3,
        (getCanvasSize() / 4) * 1,
      );
      this.sketch.text(
