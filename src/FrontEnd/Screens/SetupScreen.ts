...
export default class SetupScreen implements Menu {
  ...
  private transitionIn(): void {
    if (!this.border.isTransitioning()) {
      for (let i = 0; i < this.multiplayer_MenuButton_list.getLength(); i++) {
        if (
          (
            this.multiplayer_MenuButton_list.getAtIndex(i) as MenuButton
          ).getOpacity() <= 255
        ) {
          (this.multiplayer_MenuButton_list.getAtIndex(i) as MenuButton).fadeIn(
            (SETUP_SCREEN_ANIMATION_TIME / 3) * 2,
          );
        }
      }
      if (this.opacity < 255) {
      ...
      if (
        this.floater_array[this.floater_array.length - 1].getOpacity() >= 255
      ) {
  ...
  private transitionOut(): void {
    for (let i = 0; i < this.multiplayer_MenuButton_list.getLength(); i++) {
      if (
        this.multiplayer_MenuButton_list.getAtIndex(i) !==
        this.multiplayer_MenuButton_list.getCurrentlySelected()
      ) {
        (this.multiplayer_MenuButton_list.getAtIndex(i) as MenuButton).fade(
          255 / (MenuButton.CONFIRMED_ANIMATION_TIME / 4),
        );
      }
    }
    for (let i = 0; i < this.floater_array.length; i++) {
