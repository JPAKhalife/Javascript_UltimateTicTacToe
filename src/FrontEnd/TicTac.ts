...
export default class TicTac {
  ...
  public constructor(maxLevelSize: number = 0, gridSize = 3) {
    ...
    this.grid = Array(
      (this.GRID_SIZE * this.GRID_SIZE) ** this.maxLevelSize,
    ).fill(0);
  ...
  public selectSlot(cursorCol: number, cursorRow: number): boolean {
    if (
      this.getSlot(
        (this.selectedIndex +
          cursorCol *
            this.calculateSize(this.maxLevelSize - this.selectedLevel + 1) +
          cursorRow * this.GRID_SIZE) *
          this.calculateSize(this.maxLevelSize - this.selectedLevel + 1),
      ) < 0
    ) {
      return false;
  ...
  public updateSlot(
    ...
    if (
      this.grid[this.selectedIndex + cursorCol + cursorRow * this.GRID_SIZE] !=
      0
    ) {
      return moveResponse(TicTacState.ERROR);
    }
    this.grid[this.selectedIndex + cursorCol + cursorRow * this.GRID_SIZE] =
      turn; //Set the new slot
    //Then we check for a win
    let state = this.checkForWinOrFull(
      this.selectedIndex + cursorCol + cursorRow * this.GRID_SIZE,
    );
    ...
    let destination = this.getFirstSpot(
      this.selectedIndex + cursorCol + cursorRow * this.GRID_SIZE,
      this.maxLevelSize - state.wonLevelSize + 2,
    );
    let row = 0;
    let col = 0;
    // Now we loop into that tictac and play the equivalent
    for (
      let j = Math.max(state.wonLevelSize - 1, 1);
      ...
      let newSpot =
        destination +
        col * this.calculateSize(this.maxLevelSize - j) +
        row * this.GRID_SIZE * this.calculateSize(this.maxLevelSize - j);
      if (this.getOwner(newSpot, this.maxLevelSize - j) == 0) {
        ...
        while (
          this.getOwner(newSpot, this.maxLevelSize - this.selectedLevel + 1) !=
          0
        ) {
          this.selectedLevel--;
        }
  ...
  public getRelativeIndex(levelSize: number, index: number) {
    ...
    let size =
      (this.GRID_SIZE * this.GRID_SIZE) ** (this.maxLevelSize - levelSize);
  ...
  private checkForWinOrFull(index: number): TictacStateObject {
    ...
    for (let i = this.maxLevelSize; i >= 1; i--) {
      ...
      for (let j = 1; j < this.GRID_SIZE; j++) {
        ...
        if (
          this.getOwner(
            origin + slotRow * this.GRID_SIZE * slotSize + j * slotSize,
            this.maxLevelSize - i,
          ) !=
          this.getOwner(
            origin +
              slotRow * this.GRID_SIZE * slotSize +
              j * slotSize -
              slotSize,
            this.maxLevelSize - i,
          )
      ...
      for (let j = 1; j < this.GRID_SIZE; j++) {
        if (
          this.getOwner(
            origin + slotCol * slotSize + j * slotSize * this.GRID_SIZE,
            this.maxLevelSize - i,
          ) !=
          this.getOwner(
            origin + slotCol * slotSize + (j - 1) * slotSize * this.GRID_SIZE,
            this.maxLevelSize - i,
          )
        ) {
          isWon = false;
      ...
      if (slotCol == slotRow) {
        for (let j = 1; j < this.GRID_SIZE; j++) {
          if (
            this.getOwner(
              origin + j * this.GRID_SIZE * slotSize + j * slotSize,
              this.maxLevelSize - i,
            ) !=
            this.getOwner(
              origin +
                j * this.GRID_SIZE * slotSize +
                j * slotSize -
                slotSize -
                slotSize * this.GRID_SIZE,
              this.maxLevelSize - i,
            )
          ) {
            isWon = false;
      ...
      if (slotRow == this.GRID_SIZE - slotCol - 1) {
        for (let j = 1; j < this.GRID_SIZE; j++) {
          if (
            this.getOwner(
              origin +
                slotSize * j * this.GRID_SIZE +
                ((this.GRID_SIZE - 1) * slotSize - j * slotSize),
              this.maxLevelSize - i,
            ) !=
            this.getOwner(
              origin +
                j * slotSize * this.GRID_SIZE +
                ((this.GRID_SIZE - 1) * slotSize - slotSize * j) +
                slotSize -
                slotSize * this.GRID_SIZE,
              this.maxLevelSize - i,
            )
  ...
  public getOwner(index: number, searchingLevel: number = 1): number {
    if (this.getSlot(index) < 0 && searchingLevel > 0) {
      if (searchingLevel > Math.abs(this.getSlot(index))) {
        return 0;
      }
