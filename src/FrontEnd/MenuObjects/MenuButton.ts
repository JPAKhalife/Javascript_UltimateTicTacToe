...
export class MenuButton extends BaseMenuItem {
  ...
  private standardButton(currentCanvasSize: number): void {
    ...
    if (this.animationTime == 0) {
      ...
      this.currentWidth -=
        (this.width * (MenuButton.GROWTH_PERCENT / 100)) /
        MenuButton.SELECTED_ANIMATION_TIME;
      this.currentLength -=
        (this.length * (MenuButton.GROWTH_PERCENT / 100)) /
        MenuButton.SELECTED_ANIMATION_TIME;

      //changing the button fill to the desired fill
      this.currentButtonFill -=
        (MenuButton.SELECTED_BUTTON_SHADE - MenuButton.DEFAULT_BUTTON_SHADE) /
        MenuButton.SELECTED_ANIMATION_TIME;
      this.currentTextFill +=
        (MenuButton.SELECTED_BUTTON_SHADE - MenuButton.DEFAULT_BUTTON_SHADE) /
        MenuButton.SELECTED_ANIMATION_TIME;
    ...
    this.getSketch().rect(
      ...
      this.currentWidth * currentCanvasSize,
      this.currentLength * currentCanvasSize,
    );
  ...
  confirmedButton(currentCanvasSize: number): void {
    ...
    if (this.animationTime > MenuButton.CONFIRMED_ANIMATION_TIME) {
      ...
      this.cl +=
        ((MenuButton.GROWTH_PERCENT / 100) *
          (this.currentLength * currentCanvasSize) +
          this.currentLength * currentCanvasSize) /
        MenuButton.CONFIRMED_ANIMATION_TIME;
      this.cw +=
        ((MenuButton.GROWTH_PERCENT / 100) *
          (this.currentWidth * currentCanvasSize) +
          this.currentWidth * currentCanvasSize) /
      ...
      this.currentTextFill +=
        ...
        MenuButton.CONFIRMED_ANIMATION_TIME;
      this.setOpacity(
        this.getOpacity() - 255 / MenuButton.CONFIRMED_ANIMATION_TIME,
      );
      this.animationTime++;
    }
    this.getSketch().noStroke();
    ...
    this.getSketch().rect(
      this.getX(currentCanvasSize),
      this.getY(currentCanvasSize),
      this.currentWidth * currentCanvasSize - this.cw,
      this.currentLength * currentCanvasSize - this.cl,
    );
    this.getSketch().textSize(this.currentTextSize * currentCanvasSize);
    ...
    this.getSketch().rect(
      this.getX(currentCanvasSize),
      this.getY(currentCanvasSize),
      MenuButton.OUTLINE_WEIGHT + this.cw,
      this.cl + MenuButton.OUTLINE_WEIGHT,
    );
    this.getSketch().pop();
  }
