...
export default class MenuNav {
  ...
  constructor(itemArray: BaseMenuItem[], sketch: p5) {
    ...
    if (itemArray.length != 0) {
      ...
      this.currentlySelected = new MenuButton(
        sketch,
        0,
        0,
        "No Items",
    ...
    this.itemDistances = Array.from({ length: itemArray.length }, () =>
      Array(4).fill(0),
    );
  ...
  public mapElementLocations(): void {
    ...
    this.itemDistances = Array.from({ length: this.itemArray.length }, () =>
      Array(4).fill(0),
    );
    for (let i = 0; i < this.itemArray.length; i++) {
      let allDistancesAndDirections: {
        distance: number;
        direction: number;
        destination: BaseMenuItem;
      }[] = [];
      ...
      for (let j = 0; j < this.itemArray.length; j++) {
        if (i != j) {
          ...
          let distance = Math.sqrt(
            Math.pow(
              this.itemArray[i].getY(canvasSize) -
                this.itemArray[j].getY(canvasSize),
              2,
            ) +
              Math.pow(
                this.itemArray[i].getX(canvasSize) -
                  this.itemArray[j].getX(canvasSize),
                2,
              ),
          );
          let rawAngle =
          ...
          allDistancesAndDirections.push({
            distance: distance,
            direction: direction,
            destination: this.itemArray[j],
          });
        }
      }
