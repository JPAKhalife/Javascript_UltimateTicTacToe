...
export default class LoadingSpinner extends BaseMenuItem {
  ...
  constructor(
    ...
    yPercent: number,
    widthPercent: number,
    doRadians: boolean = false,
    rotationSpeed: number = 4,
    image: p5.Image = whiteTicTac,
    fadeFunction: ((counter: number) => number) | null = null,
    initialRotation: number = 0,
    rotationFunction: ((counter: number) => number) | null = null,
    opacity: number = 255,
  ) {
    ...
    this.rotationFunction =
      rotationFunction ||
      ((counter: number) => {
        return this.rotationAngle + this.rotationSpeed;
      });
    this.fadeFunction =
      fadeFunction ||
      ((counter: number) => {
