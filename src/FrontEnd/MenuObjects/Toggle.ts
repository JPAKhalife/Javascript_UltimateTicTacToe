...
export default class Toggle extends BaseMenuItem {
  ...
  public draw() {
    ...
    this.getSketch().rect(
      this.getX(),
      this.getY(),
      this.percentSize * 0.5 * getCanvasSize() + 2,
    );
    ...
    this.getSketch().line(
      ...
      this.getY() + ((this.percentSize / HEIGHT_RATIO) * getCanvasSize()) / 2,
      this.getX() + this.percentSize * getCanvasSize() * 0.25,
      this.getY() + ((this.percentSize / HEIGHT_RATIO) * getCanvasSize()) / 2,
    );
    this.getSketch().line(
    ...
    this.getSketch().ellipse(
      this.getX() + this.selectorPos * getCanvasSize() * this.percentSize,
      this.getY(),
      ((this.percentSize * 0.8) / 2) * getCanvasSize(),
      ((this.percentSize * 0.8) / 2) * getCanvasSize(),
    );
  ...
  private animateSlide() {
    if (this.animationProgress < ANIMATION_DURATION) {
      ...
      this.selectorPos =
        this.startPos + (this.targetPos - this.startPos) * easedT;
