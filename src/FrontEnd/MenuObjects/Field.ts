...
export default class Field extends BaseMenuItem {
  ...
  constructor(
    ...
    this.textY =
      textYPercent !== undefined
        ? textYPercent
        : yPercent - heightPercent / 2 - 0.005;
    this.textSize =
      textSizePercent !== undefined ? textSizePercent : this.fontSize * 0.8;
  ...
  public draw(currentCanvasSize?: number, keyEvent?: KEY_EVENTS): void {
    ...
    if (this.label) {
      ...
      if (this.isShaking) {
        sketch.text(
          this.label,
          this.textX * canvasSize + shakeOffsetX,
          this.textY * canvasSize,
        );
      } else {
    ...
    let textX =
      this.getX(canvasSize) -
      (this.width * canvasSize) / 2 +
      this.padding * canvasSize;
    ...
    sketch.text(
      this.errorMessage,
      this.getX(canvasSize),
      this.getY(canvasSize) + this.height * canvasSize,
    );
    sketch.pop();
    ...
    if (this.isEditing && this.cursorVisible) {
      ...
      sketch.line(
        clampedCursorX,
        textY - (this.height * canvasSize) / 4,
        clampedCursorX,
        textY + (this.height * canvasSize) / 4,
      );
    }
  ...
  private handleKeyEvent(keyEvent: KEY_EVENTS): void {
    ...
    if (
      (keyEvent === KEY_EVENTS.ENTER || keyEvent === KEY_EVENTS.SELECT) &&
      this.isSelected() &&
      this.timeSinceEnter <= 0
    ) {
